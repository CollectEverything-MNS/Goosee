...
export class DeleteProductTagController {
  ...
  @Delete(routesConfig.product.deleteProductTag.path)
  @UseGuards(JwtAuthGuard, RolesGuard)
  @Roles('products')
  @ApiOperation({ summary: "Retirer un tag d'un produit" })
  async delete(@Param('id') id: string, @Param('tagId') tagId: string) {
    const url = routesConfig.product.deleteProductTag.link(this.services.product, id, tagId);
    return this.httpProxy.delete(url, 'Delete product tag failed');
  }
}
