...
| 5.3 | `payment-service` (paiement boutique, Stripe) : microservice dédié + webhook | 3–4 j | ✅ (Stripe test câblé : PaymentIntent + Elements + webhook signé) |
...
| 6.2 | ConfigMap/Secret par tenant, `Ingress` host, cert-manager/Traefik | 2–3 j | ✅ (TLS/cert-manager hors POC local, HTTP via nip.io) |
| 6.3 | Probes HTTP, `requests`/`limits`, **HPA** | 1.5–2 j | ✅ |
| 6.4 | Jobs de migration + de seed au déploiement | 1.5–2 j | ✅ (seed OWNER via Job ; migrations jouées au boot, idempotentes) |
...
| 7.2 | Prometheus central + scrape `/metrics` et `/internal/kpi` → API d'agrégation (infra + business par tenant) | 3–4 j | ✅ (Prometheus central + agrégation KPI métier à la demande + métriques infra live) |
